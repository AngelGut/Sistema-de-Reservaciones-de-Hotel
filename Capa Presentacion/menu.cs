...
namespace Capa_Presentacion
{
    public partial class menu : MaterialForm
    {
        ...
        {
            // Verificamos si es Enter y si el Grid está en modo edición (ReadOnly = false)
            if (e.KeyCode == Keys.Enter && dgvClientes.ReadOnly == false)
            {
                e.SuppressKeyPress = true; // Evita que el Enter baje de renglón o haga "ding"
                e.Handled = true;

                // IMPORTANTE: Confirma la edición de la celda actual antes de leer los datos
                ...
                // Usamos CurrentRow que es más seguro al editar
                var fila = dgvClientes.CurrentRow;

                // Validación extra para evitar errores de nulos si la celda está vacía
                ...
                try
                {
                    ...
                    // -----------------------------------------------------------
                    // RESTAURACIÓN DE LA INTERFAZ (VITAL)
                    // -----------------------------------------------------------
                    ...
                    // 3. Restaurar comportamiento de edición (opcional, pero recomendado)
